...
import { ArrowDownCircle, CheckCircle, XCircle, Clock, AlertTriangle, Wallet, Shield } from 'lucide-react';
import { withdrawalAPI, adminAPI, authAPI } from '../../lib/api';
...
export default function WithdrawPage() {
  ...
  const totalBusinessFunds = adminStats?.totalBusinessFunds !== undefined
    ? adminStats.totalBusinessFunds
    : ((user?.walletBalance || 0) + (adminStats?.companyRevenueBalance || 0));

  const canWithdraw = isAdmin
    ? (totalBusinessFunds >= 100)
    : ((user?.walletBalance || 0) >= minBalance && (user?.qualifiedReferralsCount || 0) >= minReferrals);
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      adminAPI.dashboard()
        .then(({ data }) => setAdminStats(data.stats || null))
        .catch(() => {});
    }
  }, [isAdmin, setUser]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isAdmin) {
      if (amt > totalBusinessFunds) return toast.error('Insufficient total business funds available.');
    } else {
      if (amt > (user?.walletBalance || 0)) return toast.error('Insufficient wallet balance.');
    ...
    try {
      ...
      if (isAdmin) {
        adminAPI.dashboard().then(({ data }) => setAdminStats(data.stats || null)).catch(() => {});
      }
    } catch (err) {
    ...
    <div className="space-y-6 animate-in">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          {isAdmin && <Shield className="w-6 h-6 text-brand-400" />}
          {isAdmin ? 'Admin Business Withdrawal' : 'Withdraw Funds'}
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          {isAdmin
            ? 'Withdraw authorized admin business funds (referral wallet & company revenue)'
            : 'Request M-Pesa withdrawal to your registered number'}
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          {isAdmin ? (
            /* ADMIN / BUSINESS WITHDRAWAL INTERFACE */
            <div className="space-y-4">
              {/* Admin Total Business Funds Card */}
              <div className="p-5 bg-gradient-to-r from-emerald-950/60 via-dark-800 to-brand-950/40 border border-emerald-500/30 rounded-2xl shadow-lg">
                ...
                <p className="text-3xl font-extrabold text-white mt-2">
                  KES {(totalBusinessFunds || 0).toLocaleString()}
                </p>
                <div className="mt-3 pt-3 border-t border-slate-700/60 grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-slate-400">Referral Wallet:</span>
                    <p className="text-brand-400 font-bold text-sm">
                      KES {(adminStats?.adminReferralWalletBalance ?? (user?.walletBalance || 0)).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-400">Company Revenue:</span>
                    <p className="text-emerald-400 font-bold text-sm">
                      KES {(adminStats?.companyRevenueBalance || 0).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="card space-y-4 border border-brand-500/20">
                <h2 className="font-semibold text-white">New Business Withdrawal Request</h2>
                ...
                <div>
                  ...
                  <input
                    ...
                    min="100"
                    max={totalBusinessFunds}
                    step="1"
                    disabled={!canWithdraw}
                  />
                  <p className="text-xs text-slate-500 mt-1">
                    Draws from Admin Referral Wallet first, then Company Revenue Balance.
                  </p>
                </div>
                ...
                <button type="submit" disabled={loading || !canWithdraw} className="btn-primary w-full">
                  {loading ? (
                    ...
                    <><ArrowDownCircle className="w-4 h-4" /> Request Business Withdrawal</>
                  )}
                </button>
              </form>
            </div>
          ) : (
            /* NORMAL MEMBER WITHDRAWAL INTERFACE (UNCHANGED) */
            <div className="space-y-4">
              {/* Balance card */}
